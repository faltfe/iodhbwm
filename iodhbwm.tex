...
\documentclass[%
    ...
    language = ngerman
                ...
                Im Gegensatz zu \opt{language} wird mit der Option ausschließlich die Hauptsprache gesetzt.
    ...
    \section{Installation}
        \subsection{Lokale Installation}
            ...
            \subsubsection{TeXlive}
                \Info{Manche dieser Schritte können villeicht übersprungen werden!}
                
                \begin{enumerate}
                    \item \verb|kpsewhich -var-value TEXMFHOME| zum Auffinden des Ordner
                    \item ~/.texmf-config/tex/latex/ (ggf. neu anlegen)
                    \item Ordner iodhbwm nach ~/.texmf-config/tex/latex/ kopieren (Ordner muss zwingend iodhbwm heißen)
                    \item kpsepath -n latex tex ausführen
                    \item mktexlsr ~/.texmf-config ausführen
                    \item texhash ~/texmf ausführen
                    \item Unter umständen müssen zusätzlich noch fehlende Pakete wie z.B. lmodern nachinstalliert werden.
                \end{enumerate}
